...
final class MaterialsCompilerTests: XCTestCase {
    ...
    func testRoleDefaultsFillAbsentFieldsAndDefaultImagesAreCompiled() throws {
        ...
        XCTAssertEqual(facePNG[0].payload[0..<8].map { $0 }, [0, 0, 2, 0, 0, 0, 2, 0], "512×512")
        XCTAssertEqual(try PNGEncoder.chunks(of: out.images[1].pngData)[0].payload[0..<8].map { $0 }, [0, 0, 1, 0, 0, 0, 1, 0], "iris 256×256")
        XCTAssertEqual(out.images[0].colourSpace, .srgb)
        XCTAssertEqual(out.images[0].usage, .colour)
        XCTAssertEqual(out.rasters["image:iris"]?.width, 256)
