...
final class ProvenancePackTests: XCTestCase {
    ...
    func testSignerAvailabilityFollowsEnvironment() throws {
        ...
        // capabilities.signerAvailable and doctor.signer report the resolved credential, not a constant.
        let configured = ProjectTestHarness.context(cwd: fx.root, env: [SignerAvailability.trustPolicyKey: path, SignerAvailability.signerKey: "local-author"])
        XCTAssertTrue(DiscoveryHandlers.signerAvailable(context: configured))
        XCTAssertEqual(DiscoveryHandlers.signerReport(context: configured).json["signer"], "local-author")
        let bare = ProjectTestHarness.context(cwd: fx.root, env: [:])
        XCTAssertFalse(DiscoveryHandlers.signerAvailable(context: bare))
        let doctor = bare.registry.invoke("doctor", request: [:], context: bare)
        XCTAssertEqual(doctor.result?["signer"]?["available"], false)
        XCTAssertTrue(doctor.warnings.contains { $0.code == "SIGNER_UNAVAILABLE" })
        let doctorOK = configured.registry.invoke("doctor", request: [:], context: configured)
        XCTAssertEqual(doctorOK.result?["signer"]?["available"], true)
        XCTAssertFalse(doctorOK.warnings.contains { $0.code == "SIGNER_UNAVAILABLE" })
