...
enum DynamicPoseFactory {
    ...
    /// arm from near-down (0°) to overhead (~124°) at `f` Hz, mirroring those
    ...
    static func armSwingFast(duration: Float = 3.0, f: Float = 3.2, amplitude: Float = 62) -> AnimationClip {
        ...
        let mid = amplitude           // travels 0 → 2*amplitude (≈ 0° → 124°)
    ...
    /// from 0° to ~110° at `f` Hz. Tuned faster/wider than the first-pass
    /// (2.0 Hz / 95°) so the skirt penetrates the leg oracle over a robust
    /// multi-frame band rather than a single transient frame (#309).
    static func legMarchFast(duration: Float = 3.0, f: Float = 2.6, amplitude: Float = 55) -> AnimationClip {
        var clip = AnimationClip(duration: duration)
