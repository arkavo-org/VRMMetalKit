...
- **Frame composite.** Unchanged from the benchmark pattern: loop avatars, `drawOffscreenHeadless` into the shared MSAA texture (first clears, rest `.load`), then the existing `VRMVideoRenderer` resolve→pixelBuffer→AVAssetWriter path writes the frame.
