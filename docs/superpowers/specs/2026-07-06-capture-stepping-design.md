...
**The convergence guarantee is scoped to the committed `(captureDistance, stepDamping)` bounds.** Because `k` is known only empirically (there is no closed form — see §7), the corner test *validates* the guarantee at the **tested corner** (max `captureDistance` × min `stepDamping`, as committed) and *brackets* the `L = 1` boundary (the tested corner contracts; a just-outside point oscillates) — but it does **not locate** that boundary in parameter space. Therefore **widening either bound is a change that re-triggers the corner test at the new corner**: a raised `captureDistance` cap (or lowered `stepDamping` floor) does **not** inherit the old bounds' convergence proof, because the boundary is known only by testing and the new corner may sit outside `L < 1`. The guarantee holds for the params *as committed*, and moving the bounds toward the boundary must re-test.

---

## 7. Testing — the gate

**Validation = single-avatar headless.** This is the first *actuator*: two things can now be independently wrong — the **step logic** (trigger/placement/swing/state machine) and the **IK** (does `TwoBoneIKSolver` put the ankle where the target says) — plus the §2 feedback edge. Wiring straight into the crowd would confound all three with mutual approach + two avatars + the composite on a *visual* surface. So, exactly as increment 1 was "approve on tests, not a render," increment 2 is **approve on single-avatar headless tests**; the crowd/visible demo is a *separate step after* the controller is proven. The convergence test below is the gate that would have caught the limit cycle — it is the honest gate for this increment.
...
- **Convergence — the corner test (the gate).** Displace the CoM (a synthetic shove) and run the controller in the **arrest** regime at the **worst-case corner of the stability region — max `captureDistance` × min `stepDamping`** (max loop gain). Assert the residual imbalance sequence **contracts monotonically toward zero** (each step's residual ≤ the previous; the sequence does not grow or oscillate). This probes the corner most likely to oscillate, per §6 — not a comfortable midpoint that passes trivially. A second case just *outside* the region (loop gain > 1) is asserted to oscillate/not-contract, pinning that the test discriminates stability rather than passing unconditionally. **The convergence guarantee this test establishes is scoped to the committed `(captureDistance, stepDamping)` bounds** (§6): the test runs at the corner *those bounds define*, so widening either bound moves the corner and re-triggers this test — a later bound change inherits no proof it wasn't run for.
