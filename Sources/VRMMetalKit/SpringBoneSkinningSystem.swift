...
final class SpringBoneSkinningSystem {
    ...
    private func updateChainJointMatrices(positions: [SIMD3<Float>], nodes: [VRMNode], model: VRMModel) {
    ...
    private func calculateBoneRotation(currentDirection: SIMD3<Float>, parentNode: VRMNode, currentNode: VRMNode) -> simd_quatf {
    ...
    private func extractTwist(rotation: simd_quatf, axis: SIMD3<Float>) -> simd_quatf {
        ...
        let twistVector = SIMD4<Float>(axis.x * dotProduct, axis.y * dotProduct, axis.z * dotProduct, rotation.vector.w)
    ...
    func debugVisualizeSpringBones(model: VRMModel, renderer: VRMRenderer) {
        ...
        if let sphereColliders = buffers.sphereColliders {
            let colliders = Array(UnsafeBufferPointer(
                start: sphereColliders.contents().bindMemory(to: SphereCollider.self, capacity: buffers.numSpheres),
                count: buffers.numSpheres
            ))

            for collider in colliders {
                drawDebugSphere(center: collider.center, radius: collider.radius, color: SIMD4<Float>(0, 0, 1, 0.3))
            }
        }

        if let capsuleColliders = buffers.capsuleColliders {
            let colliders = Array(UnsafeBufferPointer(
                start: capsuleColliders.contents().bindMemory(to: CapsuleCollider.self, capacity: buffers.numCapsules),
                count: buffers.numCapsules
            ))

            for collider in colliders {
                drawDebugCapsule(p0: collider.p0, p1: collider.p1, radius: collider.radius, color: SIMD4<Float>(0, 0, 1, 0.3))
            }
        }
    }
    ...
    private func drawDebugCapsule(p0: SIMD3<Float>, p1: SIMD3<Float>, radius: Float, color: SIMD4<Float>) {
        // Implementation depends on your debug rendering system
